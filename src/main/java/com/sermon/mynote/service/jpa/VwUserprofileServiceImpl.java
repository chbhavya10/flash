...
public class VwUserprofileServiceImpl implements VwUserprofileService {
	...
	/*
	 * @Autowired private VwUserprofileRepository vwUserprofileRepository;
	 */
	...
	/*
	 * @Transactional(readOnly=true) public List<VwUserprofile>
	 * findUserProfileByUserId1(int userid){
	 * 
	 * return
	 * Lists.newArrayList(vwUserprofileRepository.findUserProfileByUserId(userid
	 * ));
	 * 
	 * }
	 */
	...
	public List<VwUserprofile> findUserProfileByUserId(int userid) {
		...
		TypedQuery<VwUserprofile> query = (TypedQuery<VwUserprofile>) em
				.createNativeQuery("select o.* from  vw_userprofile o " + " WHERE (o.UserId= :UserId) ",
						VwUserprofile.class)
